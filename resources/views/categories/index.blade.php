    ...
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="flex justify-between items-center bg-slate-800 p-4 rounded-2xl border border-slate-700">
            <h1 class="text-lg font-bold text-white flex items-center gap-2">
                <i class="fa-solid fa-tags text-blue-400"></i> بەڕێوەبردنی کاتیگۆرییەکان
            </h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-slate-800 p-5 rounded-2xl border border-slate-700 h-fit space-y-3">
                <h2 class="text-sm font-bold text-white">زیادکردنی کاتیگۆری نوێ</h2>
            ...
            <div class="bg-slate-800 p-5 rounded-2xl border border-slate-700 space-y-3">
                <h2 class="text-sm font-bold text-white">لیستی کاتیگۆرییەکان</h2>
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between items-center p-2.5 bg-slate-700/50 rounded-xl border border-slate-600">
                        ...
                        <form action="{{ route('categories.destroy', $cat->id) }}" method="POST" onsubmit="return confirm('دڵنیایت لە سڕینەوە؟')">
                            @csrf
                            @method('DELETE')
                    ...
                    @empty
                    <p class="text-xs text-slate-500 text-center py-4">هیچ کاتیگۆرییەک تۆمار نەکراوە</p>
                    @endforelse
                </div>
            </div>
        </div>
