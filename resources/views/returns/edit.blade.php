...
<head>
    ...
    <title>دەستکاریکردنی وەسڵی گەڕانەوە - {{ $return->return_no }}</title>
    ...
    <div class="max-w-6xl mx-auto space-y-5">
        ...
        <div class="flex justify-between items-center bg-[#1e293b]/90 p-4 rounded-2xl border border-slate-700/70">
            <h1 class="text-base font-bold flex items-center gap-2 text-white">
                ...
                دەستکاریکردنی وەسڵی گەڕانەوە ({{ $return->return_no }})
            </h1>
        ...
        <div class="bg-[#1e293b]/90 p-5 rounded-2xl border border-slate-700/70 grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
                ...
                <select id="customer-select" class="w-full p-2.5 bg-[#0f172a] border border-slate-700 rounded-xl text-white">
                    ...
                    @foreach($customers as $c)
                        <option value="{{ $c->id }}" {{ $return->customer_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" id="return-date" value="{{ $return->created_at->format('Y-m-d\TH:i') }}" class="w-full p-2.5 bg-[#0f172a] border border-slate-700 rounded-xl text-white font-mono">
            </div>
            ...
            <div>
                ...
                <select id="refund-type" class="w-full p-2.5 bg-[#0f172a] border border-slate-700 rounded-xl text-white font-bold text-amber-400">
                    <option value="deduct_debt" {{ $return->refund_type == 'deduct_debt' ? 'selected' : '' }}>داشکاندن لە قەرزی کڕیار</option>
        ...
        <div class="bg-[#1e293b]/90 p-5 rounded-2xl border border-slate-700/70 space-y-4">
            <div class="flex justify-between items-center">
                <h2 class="text-sm font-bold text-white">کاڵاکانی ناو وەسڵی گەڕاوە</h2>
                ...
                <table class="w-full text-xs text-center text-slate-300">
                    ...
                    <tbody id="items-table" class="divide-y divide-slate-700/50">
                        ...
                        @php
                            ...
                            $basePrice = $detail->unit_price / ($factor ?: 1);
                        @endphp
                        <tr class="item-row hover:bg-slate-800/40 transition">
                            <td class="p-2 text-right">
                                <select class="prod-select p-2 bg-slate-800 border border-slate-700 rounded-lg text-white w-full" onchange="onProductChange(this)">
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" data-price="{{ $product->base_sale_price }}" data-carton="{{ $product->kg_per_carton }}" {{ $detail->product_id == $product->id ? 'selected' : '' }}>
                                            {{ $product->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td class="p-2">
                                <select class="unit-select p-2 bg-slate-800 border border-slate-700 rounded-lg text-white w-full" onchange="calculate()">
                                    @foreach($units as $unit)
                                        <option value="{{ $unit->id }}" data-name="{{ $unit->name }}" data-factor="{{ $unit->factor_to_base }}" {{ $detail->unit_id == $unit->id ? 'selected' : '' }}>
                                            {{ $unit->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td class="p-2">
                                <select class="condition-select p-2 bg-slate-800 border border-slate-700 rounded-lg text-xs">
                                    <option value="normal" {{ $detail->condition_type == 'normal' ? 'selected' : '' }} class="text-emerald-400 font-bold">ئاسایی (دەگەڕێتەوە کۆگا)</option>
                                    <option value="expired" {{ $detail->condition_type == 'expired' ? 'selected' : '' }} class="text-rose-400 font-bold">بەسەرچوو (ناگەڕێتەوە کۆگا)</option>
        ...
        <button onclick="updateReturn()" class="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3.5 rounded-2xl transition">
    ...
    <script>
        ...
        function removeRow(btn) {
            ...
            if (rows.length > 1) {
                ...
                alert('ناتوانی هەموو دێڕەکان بسڕیتەوە');
            }
        }
